...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    train_sq = np.sum(train_sq, axis=1)
    train_pad = np.array([train_sq]*X.shape[0])
    
    test_sq = X*X
    test_sq = np.sum(test_sq, axis=1)
    test_pad = np.array([test_sq]*num_train).T
    
    third_term = X.dot(self.X_train.T)
    dists = np.sqrt(train_pad + test_pad - 2*third_term)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
